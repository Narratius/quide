...
type
  ...
  TquideValueStoryItem = class(TquideCustomStoryItem)
  private
    FValue: string;
  public
  ...
  public
    constructor Create(aParent: TquideCustomStoryItem; const aAlias, aCaption:
        string); override;
    destructor Destroy; override;
    procedure AddCondition(aCondition: TquideConditionStoryItem); overload;
    function AddCondition(aOperandA: TquideCustomStoryItem; aOperation: TquideLogicOperation; aOperandB: TquideCustomStoryItem):
        TquideConditionStoryItem; overload;
    procedure Execute(aItems: TquideStoryItemList);
    property Conditions: TquideStoryItemList read FConditions;
  end;

  TquideTextStoryItem = class(TquideStoryAction)
  private
    FText: string;
  ...
  public
    function AddText(const aText: string): TquideTextStoryItem; overload;
    function AddText(const aText: string; aOperandA: TquideCustomStoryItem; aOperation: TquideLogicOperation; aOperandB:
        TquideCustomStoryItem): TquideTextStoryItem; overload;
    function AddLogicItem(aOperandA: TquideCustomStoryItem; aOperation: TquideLogicOperation; aOperandB, aResultTrue, aResultFalse:
        TquideCustomStoryItem): TquideLogicStoryItem;
  end;
...
uses
  ...
  System.Variants, Quide.Classes.Manager;
...
begin
  l_Index:= 1;
  Result := Alias + '.' + aSuffix + l_Index.ToString;
  while HaveItem(Result) do
  begin
    Inc(l_Index);
    Result := Alias + '.' + aSuffix + l_Index.ToString;
  end;
...
begin
  ...
  if Assigned(l_Items) then
    for var i : integer := 0 to l_Items.ChildNodes.Count-1 do
    begin
      l_Node:= l_Items.ChildNodes[i];
      l_Item:= QuideClassManager.MakeStoryItem(l_Node);
...
function TquideStoryAction.AddCondition(aOperandA: TquideCustomStoryItem; aOperation: TquideLogicOperation; aOperandB:
    TquideCustomStoryItem): TquideConditionStoryItem;
begin
  Result := TquideConditionStoryItem.Create(self, DefaultAlias('Condition'), 'Условие');
...
begin
  Result := TquideParagraph.Create(Self, DefaultAlias('Paragraph'), 'Абзац');
  Add(Result);
end;
...
function TquideParagraph.AddLogicItem(aOperandA: TquideCustomStoryItem; aOperation: TquideLogicOperation; aOperandB, aResultTrue,
    aResultFalse: TquideCustomStoryItem): TquideLogicStoryItem;
begin
  Result := TquideLogicStoryItem.Create(Self, DefaultAlias('Logic'), 'Условие');
  Result.OperandA:= aOperandA;
  Result.OperandB:= aOperandB;
  Result.Operation:= aOperation;
  Result.ResultTrue:= aResultTrue;
  Result.ResultFalse:= aResultFalse;
  Add(Result);
end;

function TquideParagraph.AddText(const aText: string): TquideTextStoryItem;
begin
  Result:= TquideTextStoryItem.Create(self, DefaultAlias('Text'), 'Текст');
  Result.Text:= aText;
  Add(Result);
end;

function TquideParagraph.AddText(const aText: string; aOperandA: TquideCustomStoryItem; aOperation: TquideLogicOperation;
    aOperandB: TquideCustomStoryItem): TquideTextStoryItem;
begin
  Result := AddText(aText);
  Result.AddCondition(aOperandA, aOperation, aOperandB);
end;
