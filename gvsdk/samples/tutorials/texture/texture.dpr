...
const
  SCREEN_CAPTION = 'Texture'; //@@ screen caption
...
begin
  ...
  Texture[0] := GV_Texture_Load(RezFile, GV_ColorKey, 'media/sprites/figure.png');

  // GV can also load a texture with an alpha channel. Set colorkey to zero
  // and if the image has an alpha channel it will be loaded and applied.
  Texture[1] := GV_Texture_Load(RezFile, 0, 'media/sprites/alphacheese.png');
...
begin
  repeat
    ...
    begin
      ...
      GV_Texture_Render(Texture[0], 320-128, 240, 1.0, 0.0, GV_White, nil, GV_RenderState_Blend);
      GV_Texture_Render(Texture[1], 320+64, 240, 1.0, 0.0, GV_White, nil, GV_RenderState_Blend);

      GV_Font_Print(Font[0], 320-158, 315, GV_White, 'Color Key', []);
      GV_Font_Print(Font[0], 320+24 , 315, GV_White, 'Alpha Channel', []);
