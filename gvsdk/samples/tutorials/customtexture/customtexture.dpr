...
const
  SCREEN_CAPTION = 'Custom Texture';        //@@ screen caption
...
begin
  ...
  // Lets allocate a custom texture that is 256x256. This texture is no different
  // than a texture created with GV_Texture_Load and can be updated and rendered.
  Texture[0] := GV_Texture_Alloc(256, 256);

  // To show that its a "normal" texture, lets lock it and fill it in with WHITE
  GV_Texture_Lock(Texture[0], nil, nil);
  for y := 0 to 255 do
  begin
    for x := 0 to 255 do
    begin
      GV_Texture_SetPixel(Texture[0], X, Y, GV_White);
    end;
  end;
...
begin
  repeat
    ...
    begin
      ...
      // We want to only update the area of the texture that is WHITE
      // (255,255,255,255) set we use GV_Texture_GetPixel to return a color
      // value at point XY.
      // Note: _Set/GetPixle can only be used when a texture is locked else
      // you WILL get an exception error.
      if (GV_Texture_GetPixel(Texture[0], X, Y) = GV_White) then
      begin
      ...
      GV_Font_Center(Font[0], 390, GV_Yellow, 'Only the WHITE areas of the custom texture are being updated.', []);      
