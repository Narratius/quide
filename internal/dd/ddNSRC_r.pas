...
{ $Id: ddNSRC_r.pas,v 1.373 2013/04/11 16:46:28 lulin Exp $ }
// $Log: ddNSRC_r.pas,v $
...
// - bug fix: приведение сообщений об ошибках в AnsiCodePage
//
...
type
  ...
  private
    ...
    f_DateNums : array[TDNType] of TDateNumRec;
    ...
    procedure HeaderAddStreamAtom(Index: Long; Value: TStream);
    procedure HeaderStartChild(TypeID: Long);
    ...
    //function GetObjType(aFileName: AnsiString): TObjType;
  ...
  protected
    ...
    {-------------------------}
    function GetCurrentType: Longint;
    procedure SetCurrentType(value: Longint);
    ...
    procedure WriteParagraph(aJustifyChar: AnsiChar; aLeftIndent, aRightIndent, aFirstIndent, aFontHeight:
        Integer; aText: Tl3String; var aSubs: Tl3ObjectRefList; aTrim: Boolean; aCheckGraph: Boolean =
        True);
    procedure WriteSBSPara(Text: Tl3String; var Subs: Tl3ObjectRefList);
    procedure WriteEmptyParagraph(var aSubs:  Tl3ObjectRefList);
    ...
    procedure Cleanup; override;
    procedure CollectSubAttributes(aSub: TSub);
    procedure FinalizeDocument;
    procedure OutErrorLog(const aMsg: AnsiString);
    procedure ScanForSegment(aText: Tl3String; aTrim: Boolean; var SegList, Links,
        Objects: Tl3ProtoObjectRefList; aC: AnsiChar);

    procedure OutLeftText(const Sender: Ik2TagGenerator);
    procedure OutRightText(const Sender: Ik2TagGenerator);
    procedure StartDocument(TagID: Long; aDocType: TDocType = dtText);
...
const
  ...
  CellStartChars = [#194, #197, #203, #206, #209, #210, #215, #216];  { Начало ячеек }
  CellStoPAnsiChars  = [#193, #195, #202, #206..#208, #215, #216]; { Окончание ячеек }
  CellBreakChars = [#180, #181, #182, #185, #195, #198, #199, #204]; { Дополнительные ячейки }
  CellWhiteSpace = [#196, #205];
  CellAdvStartChars = CellStartChars+CellBreakChars+[#179,#186];
...
uses
 ...
 imageenio,
  ...
  Alarm_Const
  , l3_String, evStyleInterface, l3ObjectRefList1;

type
 TableParts = (tpTable, tpRow, tpCell);
const
 TableTypes : array[0..1, TableParts] of Integer = ((k2_idTable, k2_idTableRow, k2_idTableCell),
                                                    (k2_idSBS, k2_idSBSRow, k2_idSBSCell));
...
begin
 ...
 f_Attributes.Start;
 f_Attributes.StartChild(k2_idDocument);
 f_Style:= -1;
 f_ViewKind:= ev_bvkNone;
end;

constructor TSub.MakeBlock(aAnsiCodePage: Integer);
begin
 Create(nil);
 f_IsBlock := True;
...
constructor TSub.MakeSub(aAnsiCodePage: Integer);
begin
 Create(nil);
 f_IsBlock := False;
 AnsiCodePage:= aAnsiCodePage;
...
procedure TSub.MakeName(aText: Tl3String);
var
  j: Integer;
begin
...
procedure TSub.pm_SetANSICodePage(const aValue: Integer);
begin
 f_ANSICodePage := aValue;
 Name.CodePage:= AnsiCodePage;
end;

procedure TSub.pm_SetIsBlock(const Value: Boolean);
begin
 f_IsBlock := Value;
 f_IsValid:= True;
end;

procedure RemoveChar(aText: Tl3String; aChar: AnsiChar);
var
  i: Longint;
begin
  for i:= 0 to Pred(aText.Len) do
  begin
    if aText.Ch[i] = aChar then
      aText.Delete(i, 1);
  end;
end;

procedure SmartTrim(Text: Tl3String);
var
  i: Integer;
begin
 i:= 0;
...
procedure SmartSplit(FullText: Tl3String; var LeftText, RightText: Tl3String);
var
 i, j: longint;
 l_Text: Tl3String;
begin
  ...
  begin
   ...
   begin
    ...
    begin
     ...
     SmartTrim(LeftText); SmartTrim(RightText);
...
procedure TCustomNSRCReader.ScanForSegment(aText: Tl3String; aTrim: Boolean;
    var SegList, Links, Objects: Tl3ProtoObjectRefList; aC: AnsiChar);
var
 i: Integer;
begin
 ...
 begin
  if not f_Subs.Empty then
   OutErrorLog(Format(SNSCError_UnclosedLinkSub,
           [TopicNo,
            TSub(f_Subs.Items[f_Subs.Hi]).ID,
            TSub(f_Subs.Items[f_Subs.Hi]).Name.AsString,
...
begin
  ...
  {$IFNDEF WithoutCache}
  f_Header:= Tk2DocumentBuffer.Create(nil);
  {$ENDIF}
  f_FirstStep:= False;
  f_OnlyClasses:= False;
 ...
 f_LinkedDocs := Tl3RecList.Create(SizeOf(TDoc2DocLinkRec));
...
var
 l_ParaType: Integer;
begin
  if ((CurrentType > -1) or not StrictFormat) and not FirstStep then
  begin
    if f_SBSWrite then
      exit;
    ...
    l_ParaType:= IfThen(aIsDecor, k2_idDecorTextPara, k2_idTextPara);
...
function TCustomNSRCReader.GetCurrentType: Longint;
begin
  if FTypes.Count > 0 then
    Result:= TChildID(FTypes.Items[Pred(FTypes.Count)]).ID
  else
    Result:= -1;
end;

procedure TCustomNSRCReader.SetCurrentType(Value: Longint);
var
 C      : TChildID;
 l_Type : Tk2Type;
begin
  l_Type := TypeTable[Value];
  if not f_DocStarted and not l_Type.InheritsFrom(k2_idDocument) then
  begin
    f_DocStarted:= True;
...
procedure TCustomNSRCReader.WriteEmptyParagraph(var aSubs:  Tl3ObjectRefList);
begin
  {$IFNDEF WithoutCache}
  CheckDocHeader;
  {$ENDIF}
 ...
 begin
  CheckPara;
  if CurrentType = k2_idTextPara then
  begin
    if aSubs <> nil then
    begin
...
begin
 ...
 Sender.AddStringAtom(k2_tiText, f_Left);
end;

procedure TCustomNSRCReader.OutRightText;
begin
 f_Right.Codepage:= AnsiCodepage;
 Sender.AddStringAtom(k2_tiText, f_Right);
end;

procedure TCustomNSRCReader.WriteSBSPara(Text: Tl3String; var Subs: Tl3ObjectRefList);
begin
  while FEmptyParaCount > 0 do
  begin
    Dec(FEmptyParaCount);
    CurrentType:= k2_idTextPara;
    FinishCurrentType;
  end;
 ...
 try
  ...
  try
   f_Subs:= Subs;
   SmartSplit(Text, f_Left, f_Right);
...
procedure TCustomNSRCReader.WriteParagraph(aJustifyChar: AnsiChar; aLeftIndent, aRightIndent,
    aFirstIndent, aFontHeight: Integer; aText: Tl3String; var aSubs: Tl3ObjectRefList; aTrim:
 ...
 begin
  ...
  try
   ...
   try
     Generator.StartChild(k2_idSubLayer);
     try
       Generator.AddIntegerAtom(k2_tiHandle, ev_sbtSub);

       for i:= 0 to Pred(aSubs.Count) do
       begin
        l_Sub:= TSub(aSubs.Items[i]);

        Generator.StartChild(k2_idSub);
        try
         ...
         begin
           if l_Sub.LegalName then
             Generator.AddStringAtom(k2_tiShortName, l_Sub.Name)
           else
             Generator.AddStringAtom(k2_tiName, l_Sub.Name)
         ...
         begin
          ...
          begin
           l_Sub.Generator.Finish;
           Tk2DocumentBuffer(l_Sub.Generator).Root.Write(Generator, l3_spfAll, [k2_tiSubs]);
          end;
 ...
 procedure lp_WriteLinks(aLinks: Tl3ProtoObjectRefList);
 var
  i, j: Integer;
  l_HL: TsegHyperlink;
 begin
     ...
     try
       ...
       try
         ...
         begin
          ...
          try
           l_HL:= TsegHyperlink(aLinks.Items[i]);
           Generator.AddIntegerAtom(k2_tiStart, l_HL.Start);
           Generator.AddIntegerAtom(k2_tiFinish, Min(l_HL.Stop, aText.Len));
           for j:= 0 to Pred(l_HL.Targets.Count) do
           begin
             Generator.StartChild(k2_idAddress);
             try
              Generator.AddIntegerAtom(k2_tiDocID, TsegTarget(l_HL.Targets.Items[j]).Doc);
              if TsegTarget(l_HL.Targets.Items[j]).Sub > -1 then
                Generator.AddIntegerAtom(k2_tiSubID, TsegTarget(l_HL.Targets.Items[j]).Sub);
             finally
              Generator.Finish;
             end; // idAddress
           end;
 ...
 procedure lp_WriteObjects(aObjects: Tl3ProtoObjectRefList);
 var
  i: Integer;
 ...
 begin
   ...
   begin
     ...
     try
       Generator.StartChild(k2_idSegmentsLayer);
       try
         Generator.AddIntegerAtom(k2_tiHandle, ev_slObjects);
         ...
         begin
          ...
          try
           ...
           begin
            ...
            Generator.StartChild(k2_idFormula);
            try
             Generator.AddStringAtom(k2_tiText, l_Formula.Text);
            finally
             Generator.Finish;
            end; // idAddress
 ...
 procedure lp_WriteSegments(aSegs: Tl3ProtoObjectRefList);
 var
  i: Integer;
  l_Seg: TsegSeg;
 begin
   ...
   begin
     ...
     try
       ...
       try
         ...
         begin
          l_Seg:= TsegSeg(aSegs.Items[i]);
          if l_Seg.ColorSelection or l_Seg.HLContinue or l_Seg.Comment or
             l_Seg.Hidden or l_Seg.OutOfDate or l_Seg.NotApplied then
          begin
            Generator.StartChild(k2_idTextSegment);
            try
             Generator.AddIntegerAtom(k2_tiStart, l_Seg.Start);
             l_Seg.Stop := Min(aText.Len, l_Seg.Stop);
             Generator.AddIntegerAtom(k2_tiFinish, l_Seg.Stop);
             if l_Seg.ColorSelection then
               Generator.AddIntegerAtom(k2_tiStyle, ev_saColorSelection)
             else
             if l_Seg.HLContinue then
               Generator.AddIntegerAtom(k2_tiStyle, ev_saHyperLinkCont)
             else
             if l_Seg.Comment then
               Generator.AddIntegerAtom(k2_tiStyle, ev_saTxtComment)
             else
             if l_Seg.OutOfDate then
               Generator.AddIntegerAtom(k2_tiStyle, ev_saTxtOutOfDate)
             else
             if l_Seg.NotApplied then
               Generator.AddIntegerAtom(k2_tiStyle, ev_saNotApplied);
 ...
 begin
  with TevStyleInterface.Make(k2_idTextPara, aStyle) do
  try
...
var
  Segs, Links, Objects: Tl3ProtoObjectRefList;
  l_Seg: TsegSeg;
  F: TsegFormula;
  i,j: Integer;
  T, tmpS: Tl3String;
...
begin
 ...
 begin
  ...
  begin
    ...
    if aFontHeight <> k2_typTextPara.Prop[k2_tiPrintFontSize].DefaultValue then
   ...
   begin
     ...
     if (l_Seg.Start = 1) and (l_Seg.Stop >= aText.Len) then
     begin
       ...
       if not (l_Seg.HLContinue or l_Seg.OutOfDate or l_Seg.ColorSelection or l_Seg.Comment or l_Seg.NotApplied) and
              ((l_Seg.Start = 1) and (l_Seg.Stop >= aText.Len)) then
        Segs.Delete(0);
     end;
   end; // (Segs <> nil) and (Segs.Count > 0)
   {.$ENDIF}
  ...
  if aText <> nil then
   Generator.AddStringAtom(k2_tiText, aText);
  if aJustifyChar = 'D' then
   AddAlignBySeparator(evChar2Inch(l_Offset)); 
end;


procedure TCustomNSRCReader.CollectSubAttributes(aSub: TSub);
var
 l_Type: Long;
 l_SpecChar: AnsiChar;
begin
 while Parser.TokenChar = ';' do
 begin
  TranslateComment;
  Parser.NextTokenSp;
 end; // while Parser.TokenChar = ';'
 if Parser.TokenChar = #0 then
  exit;
   ...
   begin
    ...
    begin
      ...
      try
       ...
       try
        aSub.Generator.StartChild(k2_idDictItem);
        try
         Text.LTrim;
         aSub.Generator.AddStringAtom(k2_tiName, Text);
        finally
    ...
    begin
     ...
     begin
      aSub.Generator.StartChild(k2_idTextPara);
      aSub.Generator.Finish;
     end
     else
    ...
    begin
     TranslateComment;
     Parser.NextTokenSp;
     Parser.NextTokenSp; // итак стоим на конце строки
    end; // while Parser.TokenChar = ';'
...
begin
  ...
  {$ENDIF}
  CheckDateNums;
  l_Sub:= TSub.MakeSub(AnsiCodePage);
  try
    //Parser.NextTokenSp;
    l_Sub.ID:= GetInteger;
...
begin
 ...
 Text.Append(Filer.ReadLn);
 //l3Replace(Text.AsPAnsiCharLen, [#255], #32);
 if Convert then
  Text.CodePage:= ANSICodePage;
 Text.Trim;
end;
...
procedure TCustomNSRCReader.TranslateStyle(aIsDecor: Boolean = False);
  ...
  begin
    ...
    begin
     ...
     //l3System.Msg2Log(aText.AsString); //  !!!!!!!!!!!
    end;
  ...
  begin
   Parser.WhiteSpace:= AllWhiteSpace;
    ...
    aCP:= CodePage;
    aFontHeight:= k2_typTextPara.Prop[k2_tiPrintFontSize].DefaultValue;
    while Parser.TokenType <> l3_ttEOL do
    begin
     Parser.NextTokenSp;
     l_S:= Parser.TokenString;
...
begin
  ...
  begin
    ...
    f_Header.AddStringAtomClone(Index, Value);
  end;
  {$ENDIF}
end;
...
procedure TCustomNSRCReader.HeaderStartChild(TypeID: Long);
begin
  {$IFDEF WithoutCache}
...
begin
  ...
  try
    HeaderStartChild(k2_idDictItem);
    try
     HeaderAddStringAtom(k2_tiName, Text);
    finally
     HeaderFinish;
    end;
  finally
...
begin
 ...
 begin
  ...
  try
    f_Block.Generator.StartChild(k2_idDictItem);
    try
     f_Block.Generator.AddStringAtom(k2_tiName, Text);
    finally
   ...
   begin
    ...
    repeat
      ...
      try
       ...
       { Временный кусок }
       {$IFDEF AccGroups}
       // Было преобразование первого BELONGS  в группу доступа
       //if f_AccGroups.Empty then
       //  f_AccGroups.AsString:= Copy(tmpS, 1, i-1);
       {$ENDIF}
      finally
       HeaderFinish;
      end;
...
begin
  ...
  if Parser.TokenSymbolIs(kwdSUFFIX) then
    Text.AsString:= 'Суффиксы\';
  Text.CodePage:= cp_OEMLite;
  Text.Append(l3Trim(Filer.ReadLn));
  Text.CodePage:= AnsiCodePage;
  if not f_IsBody then
  begin
...
begin
  ...
  try
    Text.Trim;
    Text.MakeUpper;
    if Text.AsString = prclvl_nFree then
   ...
   try
    ...
    try
     if FindStreamFormat(tmp_Stream) <> ioUnknown then
      ...
      Generator.StartChild(k2_idExtDataPara);
     try
...
begin
 ...
 ReadText(True);
 if AnsiSameText(text.AsString, 'Формы документов') or AnsiSameText(Text.AsString, 'Документы ОАО "Газпром"') then
 begin
  if not f_IsBody then
  begin
...
begin
 ...
 try
  ...
  try
   ...
   begin
    Text:= Tl3String(Pubs.Items[j]);
    i:= 0;
    S:= '';
    while (Text.Ch[i] <> '|') and (i < Text.Len) do
    begin
     S:= S + Text.Ch[i];
     Inc(i);
    end; {while}
    ...
    try
     ...
     begin
      ...
      if not Text.Empty then
       HeaderAddStringAtom(k2_tiLinkComment, Text);
     end
...
begin
 ...
 ReadText(True);
 tmpS:= Text.AsString; {<-  Здесь лежит много всякой информации }
 Len:= 0;
 ...
 try
  ...
  try
    ...
    try
     ...
     Text.Delete(0, Len);
     if not Text.Empty then
      HeaderAddStringAtom(k2_tiComment, Text);
    finally
     HeaderFinish;
     l3Free(Text);
    end;
  ...
  try
    ...
    begin
      if f_Block.LegalName then
        Generator.AddStringAtom(k2_tiShortName, f_Block.Name)
      else
        Generator.AddStringAtom(k2_tiName, f_Block.Name)
    end
    else
    begin
    ...
    begin
     ...
     begin
      f_Block.Generator.Finish;
      Tk2DocumentBuffer(f_Block.Generator).Root.Write(Generator, l3_spfAll, [k2_tiSubs]);
     end;
...
begin
  ...
  ReadText(True);
  try
    { Здесь должны отрабатываться номер и имя блока }
   (*
   if not Text.Empty then
   begin
    S:= '';
...
var
  ...
  i: Integer;
begin
 if (CurrentType = -1) and not StrictFormat then
  CurrentType:= k2_idDocument;
  ...
  f_Block:= TSub.MakeBlock(AnsiCodePage);
...
begin
 ...
 try
  HeaderStartChild(k2_idLogRecord);
  try
...
begin
 Dn_Date[dnChangerDate]:= ReadDateInt('ChDate');
end;
...
begin
 ...
 try
  ...
  {$IFDEF AllComments}
    if f_DocHeaderWritten then
     if (not Text.Empty)
       and (System.Pos(UpperCase(SDeadComment), UpperCase(Text.AsString)) <> 1)
       and (System.Pos(UpperCase(SDeadComment2), UpperCase(Text.AsString)) <> 1)
       and (System.Pos(UpperCase(SDeadComment3), UpperCase(Text.AsString)) <> 1)
       and (Text.First <> chrStartToken)
       and (Text.First <> '%') then
    begin
     if CurrentType = k2_idTextPara then
     begin
  ...
  try
    ...
    begin { Разрыв раздела }
      ...
      if Text.Ch[0] = 'L' then
        ...
        OutErrorLog(Format('TOPIC# %d > Неправильная ориентация страницы %s', [TopicNo, Text.Ch[0]]));
      Text.Delete(0, 2);
      ...
      begin
        ...
        begin { Расшифровка стандартных размеров }
         ...
         try
           ...
           OutErrorLog(Format('TOPIC# %d > Неправильный формат бумаги %s', [TopicNo, Text.AsString]));
         end;
        end
        else
        if Text.Ch[0] = '"' then
        begin { Расшифровка нестандартных размеров }
          ...
          OutErrorLog(Format('TOPIC# %d > Неправильный размер бумаги %s', [TopicNo, Text.AsString]));
      end;
...
var
 ...
 function AICheck(const Data: Ik2Tag; Index: Long): Boolean;
 var
 ...
 begin
  ...
  end // not (Data.rAtom(k2_tiType).IsValid and (Data.rAtom(k2_tiType).AsLong > 0))
  else
 ...
 begin
  ...
  try
   ...
   begin
    Generator.StartChild(k2_idAddress);
    try
     with PDoc2DocLinkRec(f_LinkedDocs.ItemSlot(I))^ do
     begin
...
begin
 ...
 try
  ...
  begin
   ...
   begin
    ...
    begin
     ...
     l_Min:= maxActiveDate;
     l_Max:= minActiveDate;
     f_Header.Root.Attr[k2_tiActiveIntervals].IterateChildrenF(k2L2TIA(@AICheck));
     l_NeedActiveInterval:= l_NeedActiveInterval and (f_Header.Root.Attr[k2_tiActiveIntervals].ChildrenCount = 1);
    end
    else
     l_NeedActiveInterval:= True;
    if f_IsDictEntry then
     Generator.StartChild(k2_idDictEntry)
    else
     Generator.StartChild(f_Header.Root.TagType.ID);
    f_Header.Root.Write(Generator, l3_spfAll, [k2_tiSubs]);
    if f_IsDictEntry then
    begin
     Generator.AddStringAtom(k2_tiName, f_Termins[evd_lgRussian]);
     Generator.AddIntegerAtom(k2_tiType, Ord(dtDictEntry));
     Generator.StartTag(k2_tiShortName);
     try
      for l:= Low(TevdLanguage) to High(TevdLanguage) do
      begin
       Generator.StartChild(k2_idString);
       try
        Generator.AddStringAtom(k2_tiValue, f_Termins[l]);
       finally
        Generator.Finish;
       end; // try..finally
      end;
  ...
  if l_NeedActiveInterval and not f_Header.Root.InheritsFrom(k2_idAnnotopic) then
  begin
...
begin
 ...
 try
  HeaderStartChild(k2_idActiveInterval);
  try
   if NotSure then
...
begin
  if CurrentType <> -1 then
   l_Type := TypeTable[CurrentType];
  while (CurrentType > -1) and not l_Type.InheritsFrom(k2_idDocument) do
  begin
    OutErrorLog(Format('TOPIC# %d > Незакрытая скобка %d', [TopicNo, CurrentType]));
    FinishCurrentType;
    if CurrentType <> -1 then
     l_Type := TypeTable[CurrentType];
...
begin
 ...
 try
  Text.TrimAll;
  if (Text.First = '"') and (Text.Last = '"') then
  begin
   Text.Delete(0, 1);
   Text.Delete(Pred(Text.Len), 1);
  end; // (Text.First = '"') and (Text.Last = '"')
  if l_Date <> -1 then
   AddAlarm(l_Date, Text.AsString);
 finally
  l3Free(Text);
 end; // try..finally
end;
...
procedure TCustomNSRCReader.StartDocument(TagID: Long; aDocType: TDocType = dtText);
begin
  f_IsDictEntry:= False;
  if f_Subs <> nil then
    WriteEmptyParagraph(f_Subs);
  ...
  Text.DeleteDoubleSpace;
  {$ENDIF}
  f_IsAnno:= TagID = k2_idAnnoTopic;
  if TagID <> k2_idAnnoTopic then
   HeaderAddStringAtom(k2_tiShortName, Text);
  l3Free(Text);
  BlockCount:= 0;
  EnableComment:= True;
...
begin
 ...
 begin
  ...
  try
    ...
    try
     Text.Trim;
     HeaderAddStringAtom(k2_tiName, aPrefix + Text.AsString);
    finally
     HeaderFinish;
    end;
  finally
...
begin
 ReadText(True);
 HeaderAddStringAtom(k2_tiNameComment, Text);
end;
...
begin
  ...
  begin
   ...
   begin
     if Parser.TokenSymbolIs(kwdAnnoTOPIC) then
      StartDocument(k2_idAnnoTopic)
     else
      if Parser.TokenSymbolIs(kwdObjTOPIC) then
       StartDocument(k2_idDocument, dtObject)
      else
       if Parser.TokenSymbolIs(kwdFlashTopic) then
        StartDocument(k2_idDocument, dtFlash)
       else
        StartDocument(k2_idDocument);
   end
   else
  ...
  begin
   ...
   begin
    ...
    begin
     //l3System.Msg2Log(parser.TokenString);
     ...
     begin
       ...
       if Length(S) > 1 then
          ...
          OutErrorLog(Format('WARNING: Topic #%d > Скорее всего пропущена команда !STYLE', [TopicNo]))};
       Filer.ReadLn;
     end;
    end
   end
...
begin
 l_Zoom:= GetInteger;
 Filer.ReadLn;
 if (f_TableCount > 0) and (l_Zoom <> k2_typTable.Prop[k2_tiZoom].DefaultValue) then
...
begin
 ...
 ReadText(True);
 if l3Same(Text.AsWStr, 'BOOK', True) then
  f_UserType:= utBook
 else
 if l3Same(Text.AsWStr, 'EDITION', True) then
  f_UserType:= utEdition
 else
 if l3Same(Text.AsWStr, 'DOC', True) then
  f_UserType:= utDoc
 else
 if l3Same(Text.AsWStr, 'IZM', True) then
  f_UserType:= utIzm
 else
 if l3Same(Text.AsWStr, 'NODOC', True) then
  f_UserType:= utNoDoc
 else
 if l3Same(Text.AsWStr, 'REL', True) then
 begin
  f_IsRel:= True;
  HeaderAddIntegerAtom(k2_tiType, Ord(dtRelText));
 end
 else
 if l3Same(Text.AsWStr, 'AAKCONTENT', True) then
  f_UserType:= utAAK_Cont
 else
 if l3Same(Text.AsWStr, 'AAKTEXT', True) then
