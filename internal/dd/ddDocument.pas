...
// $Id: ddDocument.pas,v 1.216 2013/05/06 12:37:11 dinishev Exp $
...
type
  ...
  private
    ...
    f_TableOpen: Boolean;
    function AddNormalStyle: TddStyleEntry;
    procedure CorrectOneParagraphStyle(anPara : TddTextParagraph);
  ...
  public
    ...
    function ColorByIndex(Index: Longint): LongInt;
    procedure CorrectParagraphStyles;
    ...
    property TableOpen: Boolean read f_TableOpen write f_TableOpen;
...
function TddDocument.AddNormalStyle: TddStyleEntry;
var
 l_S: TddStyleEntry;
begin
 l_S := TddStyleEntry.Create;
 try
  l_S.Number := ev_saTxtNormalANSI;
  l_S.PAP.JUST := justF;
  l_S.PAP.xaFirst := def_FirstIndent;
  l_S.PAP.IsDefault := True;
  l_S.CHP.FontSize := 20;
  l_S.CHP.FontNumber:= AddFont('Arial Cyr', 204);
  l_S.CHP.FontName := FontNameByIndex(l_S.CHP.FontNumber);
  l_S.CHP.IsDefault := True;
  Result := l_S;
  f_StyleTable.Insert(0, l_S);
 finally
  l3Free(l_S);
 end;
end;
...
function TddDocument.ColorByIndex(Index: Longint): LongInt;
begin
 if InRange(Index, 0, f_ColorTable.Hi) then
  Result:= TddColorEntry(f_ColorTable.Items[Index]).Color
 else
 begin
  Result:= 0;
  l3System.Msg2Log('Запрошен некорректный цвет %d из %d', [Index, f_ColorTable.Count]);
 end;
end;

procedure TddDocument.CorrectOneParagraphStyle(anPara : TddTextParagraph);
var
  l_S: TddStyleEntry;
  l_Seg: TddTextSegment;
  j: Integer;
begin
  if not InRange(anPara.PAP.Style, -1, 0) then
  begin
   l_S:= StyleTable[anPara.PAP.Style];
   if l_S <> nil then
   begin
    if l_S.StyleDef = sdParagraph then
     anPara.PAP.{MergeWith}InheriteFrom(l_S.PAP);   // было Join
    anPara.CHP.{MergeWith}InheriteFrom(l_S.CHP);
   end; // l_S <> nil
  end; // not InRange(l_P.PAP.Style, -1, 0)
  for j:= 0 to Pred(anPara.SegmentCount) do
  begin
   l_Seg:= anPara.Segments[j];
   if not InRange(l_Seg.CHP.Style, -1, 0) then
   begin
    l_S:= StyleTable[l_Seg.CHP.Style];
    if l_S <> nil then
    begin
     l_Seg.CHP.InheriteFrom(l_S.CHP);//MergeWith(l_S.CHP);
     l_Seg.CHP.Style:= l_S.Number;
    end; // l_S <> nil
   end; // not InRange(l_Seg.CHP.Style, -1, 0)
  end; // for j
 //anPara.PAP.IsDefault:= True;
 //anPara.CHP.IsDefault:= True;
end;

procedure TddDocument.CorrectParagraphStyles;
var
  i   : Integer;
  l_P : TddTextParagraph;
begin
  ...
  begin
   if TddDocumentAtom(f_Paragraphs.Items[i]).AtomType = dd_docTextParagraph then
   begin
    l_P:= TddTextParagraph(f_Paragraphs.Items[i]);
    //CorrectOneParagraphStyle(l_P);
   end;
  end; // for i
end;
...
var
  ...
  function FindNearestPara: TddTextParagraph;
  begin
   // неплохой код, однако...
    Result:= nil;
  end;
...
begin
 ...
 begin
  ...
  case l_A.AtomType of
    ...
    dd_docTableRow      : Result:= TddTableRow(l_A).LastCell.LastPara;
    dd_docTable         : Result:= TddTable(l_A).LastRow.LastCell.LastPara;
    dd_docParaList      : Result:= TddParaList(l_A).LastPara;
    dd_docBreak         ,
    dd_docField         : Result:= FindNearestPara;
    else
    begin
     Assert(false);
     Result := nil;
...
begin
  ...
  // напильник
  (*
  if (Result = nil) and(No = ev_saTxtNormalANSI) then
   Result := AddNormalStyle;
  *) 
...
begin
 if (not f_Paragraphs.Empty) and (f_Paragraphs.Last is TddTable) and (not TddTable(f_Paragraphs.Last).Closed) then
  Result:= TddTable(f_Paragraphs.Last)
 else
  Result := nil;
end;
...
begin
  for i:= 0 to f_Paragraphs.Hi do
    TddDocumentAtom(f_Paragraphs.Items[i]).Write2Generator(Generator, LiteVersion);
end;
...
begin
 ...
 begin
  ...
  begin
   //CorrectOneParagraphStyle(TddTextParagraph(aAtom));// Зачем это?
   l3Set(f_LastPara, TddTextParagraph(aAtom));
  end
  else
   l3Free(f_LastPara);
  if f_Generator.Generator <> nil then
   aAtom.Write2Generator(f_Generator.Generator)
  else
     ...
     k2_tiJustification:
      begin
       ...
       begin
        ...
        f_StyleEntry.CHP.FontSize:= 2*11; // заточка
       ...
       begin { обработка стиля }
        ...
        try
         ...
         begin
          ...
          if f_StyleRt.StyleDef = sdParagraph then
           l_PAP.Style:= Value.AsInteger
          else
          if f_StyleRt.StyleDef = sdCharacter then
        ...
        begin
          ...
          begin
            ...
            begin
             ...
             f_StyleEntry.CHP.FontSize:= 2*11;// Заточка K379236513
...
begin
  ...
  If (AtomIndex = k2_tiShortName) then
   If (Value.Kind=k2_vkString) then
    ...
    ConvertErrorEx(Value.Kind)
  { Может быть, это уже и не нужно?
  else
   If (AtomIndex = k2_tiName) then
    If (Value.Kind=k2_vkString) then
     Begin
      Value.AsString.CharSet:= l3_csOEM;
      f_SubRec.Name:= Value.AsString.AsString;
      f_SubRec.RealName:=False;
     end
    else
     ConvertErrorEx(Value.Kind);
  }
...
begin
  ...
  if OneStep then
  begin
   // Получаем таблицу стилей и таблицу цветов
   { TODO -oДудко Дмитрий -cРазвитие : Получить таблицы цветов, шрифтов и стилей }
   // таблица стилей добавляется после старта документа
   // AddStyleTable;
  end;//OneStep  
end;
...
begin
 ...
 begin
  ...
  f_Document.CorrectParagraphStyles;
  WriteDocument;
 end
 else
  WriteDocumentEnd;
      ...
      begin
       ...
       begin
        ...
        begin
         ...
         if l_Cell.Items[l_ParaIndex].AtomType = dd_docPicture then
         begin
         end;
        end;
       end;
    ...
    begin
     ...
     for j:= 0 to l_Row.CellCount-1 do
      ...
      begin
       ...
       (*
        if (Table.CellsProps[k, j] <> nil) and
            Table.CellsProps[k, j].VMerged then
          l_Row.Cells[j].Props.RowSpan:= l_Row.Cells[j].Props.RowSpan+1
        else
         break;
        *)
...
begin
  ...
  try
   ...
   f_BorderOwner := boRow;
   l_Row.TAP.Border.IsFramed:= not Table.IsSBS;
   Table.AddRow(l_Row);
  finally
   FreeAndNil(l_Row);
  end;
