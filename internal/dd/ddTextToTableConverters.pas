 ...
 try
  ...
  begin
   ...
   if aCell.Index > 0 then
    Dec(l_Len, f_Row.Cells[aCell.Index - 1].Props.Width);
...
//#UC START# *4FACE8BC035B_4DCA38AF0022_impl*
 ...
 begin
  ...
  begin
   ...
   if ApplyFormatting(aCell.LastPara, aCell) then
   begin
    AdjustTrimAll(aCell.LastPara, aText);
    AddText(aText);
...
//#UC START# *4FACE8DF0349_4DCA38AF0022_impl*
 l_NewPara := not aCell.LastPara.Empty;
 l_Start := aStart;
 if l_NewPara then
  l_Para := TddTextParagraph.Create(nil)
 else
 begin
  l_Para := aCell.LastPara;
  Inc(l_Start);
 end;
 try
...
//#UC START# *4FACFE6F02DF_4DCA38AF0022_impl*
 ...
 begin
  if aCell.Index = 0 then // Добавляем метки
    ...
    begin
       ...
       begin
         ...
         begin
          ...
          begin
           ...
           begin
            if f_Row.LastCell.LastPara.Empty then
             f_Row.LastCell.LastPara.Assign(TPersistent(l_TextList.Items[i]))
            else
     ...
     begin
       ...
       begin
        ...
        if l_Cell <> nil then
        begin
         if not l_Cell.LastPara.Empty then //В каждой ячейке много абзацев
...
//#UC START# *4FACEC7801A0_4DCA38C7030D_impl*
 ...
 begin
  ...
  begin
   l_Cell := f_Row.Cells[i];
   if (l_Cell.Count = 1) and l_Cell.LastPara.Empty and f_LogicalTable and
...
//#UC START# *4FACEDA302F4_4DCA38EB0022_impl*
 ...
 begin
  l_Cell := f_Row.Cells[i];
  if (l_Cell.Count > 1) or not l_Cell.LastPara.Empty then
  begin
   l_RowNotEmpty := True;
   Break;
  ...
  begin
   ...
   begin
    ...
    repeat
     ...
     if (l_Cell <> nil) and l_Cell.LastPara.Empty then
      l_Cell.LastPara.AddText(cc_SoftSpace, CP_ANSI);

     l_PrevPos := l_Pos + 1;
    until (l_Pos = -1) or (l_Pos > l_Text.Len); // repeat..until
    if l_Pos <> -1 then
     for i := f_Row.CellByPos[l_Pos].Index + 1 to f_Row.CellCount - 1 do
     begin
      with f_Row.Cells[i] do
      begin
       AddParagraph;
       LastPara.AddText(cc_SoftSpace, CP_ANSI);
      end; // with f_Row.Cells[i] do
     end; // for i := f_Row.CellByPos[l_Pos].Index + 1 to f_Row.CellCount - 1 do
   end; // l_Text.Empty
  end; // for l_Line
  f_RowList.Add(f_Row);
  l3Free(f_Row);
      ...
      try
       ...
       try
        ...
        begin
         ...
         f_Row.LastCell.LastPara.AddText(Copy(l_Txt.AsString, 0, l_Pos), l_Txt.CodePage);
         l_Txt.Delete(0, l_Pos+1);
        end; // while SepS.BMSearchUC(l_Txt, l_BT, l_Pos) do
        f_Row.AddCellAndPara;
        f_Row.LastCell.Props.Width := l_Txt.Len;
        Inc(l_RowLen, f_Row.LastCell.Props.Width);
        f_Row.LastCell.LastPara.AddText(l_Txt);
       finally
        l3Free(l_Txt);
       end; // l_Txt
       f_RowList.Add(f_Row);
