...
begin
 ...
 {$ENDIF}
  ...
  try
    if (RDS in CollectibleRDS) and (RDS <> aNewRDS) and Destination[RDS].LastParagraph.Empty{<= интересно, почему это было отключено? } then
    begin
      if Destination[RDS].LastParagraph.DeleteLastSegment then
      // Destination[RDS].StandingList.DeleteLast; { TODO : Зачем удалялось состояние? }
    end;
  except
   on E: Exception do
   ...
   rdsPicture:
    ...
    begin
     ...
     if (f_PictureEntry <> nil) then
      FreeAndNil(f_PictureEntry); { Это временно. Интересно, насколько это временно? }
     f_PictureEntry:= TddPicture.Create(nil);
     //f_PictureEntry.PAP.Assign(PAP);{ TODO : Восстановить (перенести в TdestNorm) }
     *)
    end;
   (* Обработка листов перенесена на destination
   rdsList:
    begin
 ...
 try
  ...
  begin
   if l_OldDest is TdestNorm then
    ...
    (* Нужно проверить как это теперь работает
   ...
   // l_OldDest.clear; <- Очищать нужно в месте использования
