...
{ $Id: ddNSRC_w.pas,v 1.305 2013/04/10 16:00:52 lulin Exp $ }
...
type
  ...
  TddNSRCGenerator = class({$IfDef evNSRCPlus}
                           TevNSRCPlusWriter
                           {$Else  evNSRCPlus}
                           TevCustomNSRCWriter
                           {$EndIf evNSRCPlus}
...
//    f_NotVIncluded: Boolean;
  ...
  private
    ...
    function CheckValueType(const Value: Tk2Variant; aTypeKind : Tk2VarKind): Boolean;
                           procedure ClearHeader(aTypeID: Integer);
    procedure ClearPIR;
...
(*    procedure DoOutHyperlinkEx(const Ob: Ik2Tag; var S: AnsiString);
    ...
    //property VAnoncedDateStr: AnsiString
    //  read f_VAnoncedDateStr;
...
procedure TddNSRCGenerator.StartChild(TypeID: Long);
const
...
begin
  ...
  if (not (l_TT = nil)) and l_TT.Inheritsfrom(k2_idDocument)
  ...
  inherited;
  l_TT:= CurrentType;
  if l_TT.InheritsFrom(k2_idActiveInterval) and (TopObject[1].isProp) then
    l3FillChar(f_ActiveInterval, SizeOf(TActiveIntervalRec), 0)
   else
  if l_TT.InheritsFrom(k2_idAlarm) and (TopObject[1].isProp) then
    l3FillChar(f_Alarm, SizeOf(TAlarmRec), 0)
  else
  if l_TT.InheritsFrom(k2_idAddress) and TopObject[1].IsProp and
     (TopObject[1].AsProp.TagIndex = k2_tiLinkedDocuments) then
    l3FillChar(f_Doc2DocRec, SizeOf(TDoc2DocLinkRec), 0){
  else
  Begin
   if (f_DictRec.DictID = dlBases) and (TopObject[1].IsProp) and
      (TopObject[1].AsProp.TagIndex <> k2_tiGroups) and not f_BelongsDone then
    AddBelongs;
  end};
   {$IFDEF AccGroups}
   if (f_DictRec.DictID = dlAccGroups) and (TopObject[1].IsProp) and
    ...
    begin
     ...
     f_DictRec.DictID:= dlNone;
     f_AccGroupsDone:= True;
    end;
    {$ENDIF}
   If (l_TT.InheritsFrom(k2_idSub) and
      Not l_TT.InheritsFrom(k2_idDocumentSub) and
      not l_TT.InheritsFrom(k2_idBookmark)) then
    Begin
   ...
   If (l_TT.InheritsFrom(k2_idBlock)) then{ and
      Not TT.InheritsFrom(k2_idDocumentSub))}
    Begin
       If (f_Block.ID > 0) then
        AddBlock
    end
   else
    If l_TT.InheritsFrom(k2_idDictItem) and // Было DictRec
       (TopObject[1].isProp) then
     Begin
      case TopObject[1].AsProp.TagIndex of
        k2_tiGroups: begin
                       if f_DictRec.DictID <> dlBases then
                       begin
                         f_Belongs.Clear;
                         f_DictRec.DictID:=dlBases;
                       end;
                     end;
        k2_tiSources: f_DictRec.DictID := dlSources;
        k2_tiTypes : f_DictRec.DictID := dlTypes;
        k2_tiClasses : f_DictRec.DictID := dlClasses;
        k2_tiKeyWords: f_DictRec.DictID := dlKeywords;
        k2_tiWarnings : f_DictRec.DictID := dlWarnings;
        k2_tiPrefix: f_DictRec.DictID := dlPrefixes;
        k2_tiTerritory: f_DictRec.DictID := dlTerritories;
        k2_tiNorm : f_DictRec.DictID := dlNorms;
        k2_tiAnnoClasses : f_DictRec.DictID := dlAnnoClasses;
        {$IFDEF AccGroups}
        k2_tiAccGroups:
                      begin
                       if f_DictRec.DictID <> dlAccGroups then
                       begin
                         f_AccGroups.Clear;
                         f_DictRec.DictID:=dlAccGroups;
                       end;
                      end;
         {$ENDIF}
        k2_tiServiceInfo : f_DictRec.DictID := dlServiceInfo;
      end;
   end;
...
begin
 ...
 {$ELSE}
 TT:= CurrentType;
 if TT.InheritsFrom(k2_idTextPara) and f_FirstPara and (AtomIndex = k2_tiStyle) then
   ApplyToFirstPara(Value);
 if not ((AtomIndex = k2_tiShortName) and TT.InheritsFrom(k2_idDocument)) then
 ...
 If TT.InheritsFrom(k2_idDocument) then {заполнение элементарных полей документа}
  ApplyToDocument(AtomIndex, Value)
 else { Document }
 if TT.InheritsFrom(k2_idBlock) then
  ApplyToBlock(AtomIndex, Value)
 else
 if (TT.InheritsFrom(k2_idBitmapPara) or TT.InheritsFrom(k2_idExtDataPara)){ and (not NotAnObject)} then
  ApplyToBitmapPara(AtomIndex, Value)
 else
 if TT.InheritsFrom(k2_idSub, [k2_idDocumentSub]) then  {заполнение записи: точка входа (Sub)}
  ApplyToSub(AtomIndex, Value)
 else
 if (AtomIndex = k2_tiValue) and TT.InheritsFrom(k2_idString) then
 ApplyToString(Value)
 else
 if TT.InheritsFrom(k2_idActiveInterval) then
  ApplyToActiveInterval(AtomIndex, Value)
 else
 if TT.InheritsFrom(k2_idAlarm) then
  ApplyToAlarm(AtomIndex, Value)
 else
 if TT.InheritsFrom(k2_idAddress) and TopObject[1].IsProp and (TopObject[1].AsProp.TagIndex = k2_tiLinkedDocuments) then
  ApplyToAddress(AtomIndex, Value)
 else
 if TT.InheritsFrom(k2_idDictRec) then {заполнение записи: элемент словаря}
  ApplyToDictRec(AtomIndex, Value)
 else
 if TT.InheritsFrom(k2_idStage) then {заполнение записи: этап обработки}
  ApplyToStage(AtomIndex, Value)
 else
 if TT.InheritsFrom(k2_idNumAndDate) then {заполнение записи: дата.номер.комментарий}
  ApplyToNumAndDate(AtomIndex, Value)
 else
 if TT.InheritsFrom(k2_idCheck) then {заполнение записи: элемент вычитки}
  ApplyToCheck(AtomIndex, Value)
 else
 if TT.InheritsFrom(k2_idPIRec) then {заполнение записи: элемент публикации}
  ApplyToPublished(AtomIndex, Value)
 else
 if TT.InheritsFrom(k2_idLogRecord) then {заполнение записи: элемент журнала}
...
begin
 ...
 try
  ...
  begin
   ...
   if TT.InheritsFrom(k2_idExtDataPara) then
   {$ifNDef NoObjTopic}
    WriteExtDataPara
   {$EndIf NoObjTopic} 
   else
   ...
   if TT.InheritsFrom(k2_idSub) and
      Not TopType[0].InheritsFrom(k2_idDocumentSub) and
      not TopType[0].InheritsFrom(k2_idBookmark) then
   begin
    if (f_SubRec.ID > 0) then
    begin
     AddSubRec;
   ...
   if TT.InheritsFrom(k2_idDictRec) then
   begin
   ...
   if TT.InheritsFrom(k2_idStage) then
    AddStageRec
   else
   ...
   if TT.InheritsFrom(k2_idCheck) then
    AddCheckRec
   else

   if TT.InheritsFrom(k2_idPIRec) then
    AddPublInString
   else

   if TT.InheritsFrom(k2_idLogRecord) then
    AddLogRec
   else

   if TT.InheritsFrom(k2_idAddress) and TopObject[1].IsProp and
      (TopObject[1].AsProp.TagIndex = k2_tiLinkedDocuments) then
    AddDoc2DocLink
...
begin
 if NotAnObject and not (f_UserType in NoDocs) then
  ...
  begin
   ...
   begin
    ...
    case DictID of
      dlSources: //LS(Format('!%s %s', [kwdSource, Name.AsString]));
        OutCommand(kwdSource);
      dlTypes: //LS(Format('!%s %s', [kwdType, Name.AsString]));
        OutCommand(kwdType);
      dlClasses:
         if not Name.Empty then
         begin
      ...
      dlPrefixes: //LS(Format('!%s %s',[kwdPrefix, Name.AsString]));
       OutCommand(kwdKind);
      dlTerritories: //LS(Format('!%s %s',[kwdTerritory, Name.AsString]));
        OutCommand(kwdTerritory);
      dlNorms: //LS(Format('!%s %s',[kwdNorm, Name.AsString]));
       OutCommand(kwdNorm);
      dlServiceInfo : // LS(Format('!%s %s',[kwdServiceInfo, Name.AsString]));
       OutCommand(kwdServiceInfo);
    end; { case }
    if DictID in [dlSources, dlTypes, dlClasses, dlPrefixes, dlTerritories, dlNorms, dlServiceInfo] then
     OutString(Name);
   end; // not
   case DictID of
    dlAnnoClasses : WriteAnnoClass;
    dlWarnings: //LS(Format('!%s %d', [kwdWarning, Handle]));
     begin
    ...
    dlCorSources:
      Begin
        If (f_DictRec.Handle<>0) and
           TopType[1].InheritsFrom(k2_idCheck) then
              f_CheckRec.Sour:= f_DictRec.Handle;
       end;
    dlNorms: DictID:= dlNone;
    else
      if UserDict then
      Begin
       If Handle <> 0 then
       Begin
        If TopType[1].InheritsFrom(k2_idCheck) then
         f_CheckRec.User:= Handle;

        If TopType[1].InheritsFrom(k2_idStage) then
         f_StageRec.User:= Handle;

        If TopType[1].InheritsFrom(k2_idLogRecord) then
         f_LogRec.rAuthor := Handle;
       end;
     end
   end;
  end { with }
 ...
 if (f_DictRec.DictID in [dlWarnings, dlServiceInfo]) and (f_UserType in NoDocs) then
  if (f_DictRec.DictID = dlWarnings) then
  ...
  if (f_DictRec.DictID = dlServiceInfo) and not f_DictRec.Name.Empty then
 ...
 begin
  Name.Clear;
  if (DictID <> dlBases) and (DictID <> dlAccGroups) then
   DictID:= dlNone;
  ShortName.Clear;
 end;
end;
...
begin
 if NotAnObject and not (f_UserType in NoDocs) then
    ...
    case DNType of
      ...
      dnPublish:
       begin
        ...
        begin
         OutCommand(kwdDate);
         OutDate(Date);
         f_MinDate:= Min(f_MinDate, Date);
        end;
        if not Number.Empty then
        begin
      ...
      dnMU :
       begin
         ...
         if (Date = 0) and Number.Empty then
  ...
  if (f_DateNumRec.DNType = dnChangerDate) and
     (f_UserType = utEdition) and (f_DateNumRec.Date > 1) then
       begin
        OutCommand(kwdChDate);
        OutDate(f_DateNumRec.Date);
       end;
  with f_DateNumRec do
  begin
    Comment.Clear;
...
{ TODO : Фигня какая-то, а не метод.... }
  if aName.Empty then
    exit;
  if f_DocumentType = k2_idAnnoTopic then
   l_Kwd := kwdAnnoName
  else
   l_Kwd := kwdName;
  ...
  Byte(dtFlash):
   ...
   begin
    ...
    begin
     ...
     begin
      if f_DocumentType = k2_idDocument then
      begin
       OutCommand(kwdTopic);
       OutInteger(f_Handle);
       OutSpace;
...
begin
 If f_DocIsStarted and (f_Priority <> -1) and NotAnObject and (f_UserType in Docs) then
 begin
...
begin
 if NotAnObject then
 begin
  if f_DocumentType = k2_idDictEntry then
   f_UserType:= utDoc;
  {$IFDEF DocType}
...
(*
 Inc(f_PicNumber);
 l_St:= l3ANSI2OEM(Format('Рисунок %d', [f_PicNumber]));
 // 1. Добавить в генератор Абзац "См. граф. объ"
 LS('!STYLE L 1 72 0');
 LS(l3ANSI2OEM(#5#3'См. '#4'графический объект'#4'%d'#4#3#5), [f_PictHandle]);
*)
 ...
 try
  ...
  l_PictFileName:= Lowercase(ConcatDirName(ConcatDirName(ExportDirectory, l_Topic), l_PictFileName));
  ...
  try
   ...
   (*
   // 3. Создать файл с обътопиком
   //
   //!OBJTOPIC 3812169 УНИФИЦИРОВАННАЯ ФОРМА N КО-1
   //!NAME Унифицированная форма N КО-1 "Приходный кассовый ордер" (утв.
   //!NAME постановлением Госкомстата РФ от 18 августа 1998 г. N 88)
   //!BELONGS OEM\PRA\FOR\GAR
   //!OBJTYPE RTF
   //!OBJPATH 12013060\ko-1.rtf
   //
   with Tl3TextStream.Create(ConcatDirname(ExportDirectory, IntToStr(f_PictHandle)+'.nsr'), l3_fmWrite)do
   try
    FormatLn('!%s %d %s', [kwdObjTopic, f_PictHandle, l_St]);
    FormatLn('!%s %s', [kwdNAME, l_St]);
    if not f_Belongs.Empty then
     FormatLn('!%s %s', [kwdBelongs, f_Belongs.AsString]);
    FormatLn('!%s PNG', [kwdObjType]);
    FormatLn('!%s %d\%s', [kwdObjPath, f_Handle, LowerCase(ExtractFileName(l_PictFileName))]);
   finally
    Free;
   end;
   *)
...
begin
 case AtomIndex of
  k2_tiType:
    If (Value.Kind=k2_vkInteger) then
     f_DateNumRec.DNType:=TDNType(Value.AsInteger)
    else
     ConvertErrorEx(Value.Kind);
  k2_tiStart:
     If (Value.Kind=k2_vkInteger) then
       f_DateNumRec.Date:=TStDate(Value.AsInteger)
     else
      ConvertErrorEx(Value.Kind);
  k2_tiNumber:
      If (Value.Kind=k2_vkString) then
       f_DateNumRec.Number.Assign(Value.AsString)
      else
       ConvertErrorEx(Value.Kind);
  k2_tiComment:
       If (Value.Kind=k2_vkString) then
        f_DateNumRec.Comment.Assign(Value.AsString)
       else
        ConvertErrorEx(Value.Kind);
 end // case AtomIndex;
end;
...
procedure TddNSRCGenerator.ClearHeader(aTypeID: Integer);
begin
