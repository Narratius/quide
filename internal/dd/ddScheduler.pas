...
{ Автор: Дудко Д.В.  }
{ Начат: 11.11.2003  }
{ $Id: ddScheduler.pas,v 1.62 2013/04/11 16:46:28 lulin Exp $ }
...
Uses
 ...
 l3RecList,
 l3LongintList,
 ddAppConfigTypes, ddAppConfigConst;
...
type
 ...
 private
    ...
    procedure TypeChanged(aItem: TddBaseConfigItem; aValue: TddConfigValue);
 protected
...
begin
 ...
 begin
  ...
  try
   ...
   l_CI:= TddContainerConfigItem.Make('Periodicity', 'Когда');
    l_CI.Labeled:= False;
    ...
    l_CI.AddCase('Каждые N минут');
     ...
     l_CI.Add(TddIntegerConfigItem.Create('Minutes', 'каждые|минут', 30));
...
procedure TddSchedulerTask.TypeChanged(aItem: TddBaseConfigItem; aValue: TddConfigValue);
begin
...
begin
 Result:= False;
 l3System.Msg2Log('Выполняется задача: %s', [aTask.Caption]);
...
begin
 ...
 begin
  ...
  l3System.Msg2Log('Следующая задача: %s, запуск %s', [f_CurrentTask.Caption, l_Day]);
