...
{ $Id: ddMisspellCorrect.pas,v 1.20 2013/04/11 16:46:28 lulin Exp $ }
...
type
 ...
 private
  ...
  procedure DoReplace(const aPara: Ik2Tag; aBlockLog: Boolean); overload;
...
var
 l_Value: Tk2Variant;
begin
 if CheckNames and CurrentType.InheritsFrom(k2_idSub) then
 begin
  case AtomIndex of
   k2_tiHandle:
   ...
   k2_tiName, k2_tiShortName:
    begin
     ...
     l_Value.Kind := k2_vkString;
     l_Value.AsString := Tl3String.Make(DoReplace(Value.AsString.AsWStr, True).AsWStr);
     try
      inherited AddAtomEx(AtomIndex, l_Value);
      Exit;
     finally
      FreeAndNil(l_Value.AsString);
     end;
    end;
  end; // case
 end;
...
var
 ...
 l_Str: AnsiString;
begin
 if CurrentType.InheritsFrom(k2_idDocument) and NeedErrorsLog then
 begin
...
procedure TddMisspellCorrectFilter.DoWritePara(const aLeaf: Ik2Tag);
begin
...
procedure TddMisspellCorrectFilter.DoReplace(const aPara: Ik2Tag; aBlockLog: Boolean);
var
 I: Integer;
 l_Para: InevPara;
...
begin
 if f_Searcher.HasData and aPara.QT(InevPara, l_Para) then
 begin
...
procedure TddMisspellCorrectFilter.pm_SetOnError(const Value: TddErrorEvent);
begin
 f_OnError:= Value;
end;
...
function TddMisspellSearcher.DoCheckText(const aPara : Ik2Tag;
                                               aText : Tl3CustomString;
                                         const  aSel : TevPair;
                                         out  theSel : TevPair): Bool;
//        override;
var
 ...
 var
  ...
  function lp_CheckSegmant(const aSeg: Ik2Tag; anIndex: Long): Boolean;
  begin
 ...
 begin
  ...
  begin
   ...
   evSegmentsLayer_IterateSegmentsF(aPara, l_Layer, theSel.rStart, theSel.rFinish, k2L2TIA(@lp_CheckSegmant))
...
begin
 ...
 begin
  ...
  while True do
  begin
   Result := f_SearchTree.Search(l_Str, l_Pos, f_FoundData);
   if Result then
   begin
...
procedure TddMisspellSearcher.pm_SetOnError(const Value: TddErrorEvent);
begin
 f_OnError:= Value;
end;
