...
{ $Id: ddImportPipe.pas,v 1.81 2013/04/15 06:54:34 narry Exp $ }
...
type
  ...
  private
   ...
   function _UpdateDicts(aDictType: TDLType; aDocID, aSubID: TSubID;
                          aName: Tl3CustomString): Boolean;
   procedure _StartTopic(aTopicNo: Longint; aMessage: AnsiString);
  ...
  TddImportPipe = class(TPipeDream)
  end;

  {Базовый класс для создания цепочек импорта }
  TBaseImportPipe = class(Tl3Base)
  private
    f_Filter: Tk2TagFilter;
    f_Progressor: TddProgressObject;
    f_Reader: Tk2CustomReader;
    f_Writer: Tk2TagGenerator;
  public
    function Execute: Boolean;
    property Filter: Tk2TagFilter read f_Filter write f_Filter;
    property Progressor: TddProgressObject read f_Progressor write f_Progressor;
    property Reader: Tk2CustomReader read f_Reader write f_Reader;
    property Writer: Tk2TagGenerator read f_Writer write f_Writer;
  end;


  TFileImportPipe = class(TBaseImportPipe)
  private
    f_FileList: TStrings;
    procedure pm_SetFileList(const Value: TStrings);
  public
    property FileList: TStrings read f_FileList write pm_SetFileList;
  end;

  TNSRCImportPipe = class(TFileImportPipe)
  end;

const
 ddAllDicts = [dlSources,dlTypes,dlClasses,dlKeyWords,dlBases,
...
function TPipeDream._ProcessOneFile(const aFileName: AnsiString): Boolean;
var
 ...
 *)
 ...
 begin
   ...
   if MoveFiles then
    ClearDoneDir;
   l_OldIdle := GlobalHTServer.OnIdle;
   GlobalHTServer.OnIdle := _HTIdle;
   try
    if PartSize > 0 then
    ...
    GlobalHTServer.OnIdle := l_OldIdle;
   end;
...
procedure TPipeDream.AddError(const Msg: AnsiString; Category: Integer = 0);
begin
 try
  f_ErrorLog.Add(Msg);
 except
...
procedure TPipeDream._StartTopic(aTopicNo: Longint; aMessage: AnsiString);
var
 l_Msg : AnsiString;
begin
...
procedure TPipeDream.SetFileMask(aValue: AnsiString);
begin
 if f_FileIterator = nil then
  f_FileIterator:= TddFileIterator.Create;
...
function TPipeDream.GetFileMask: AnsiString;
begin
 if f_FileIterator = nil then
  f_FileIterator:= TddFileIterator.Create;
...
function TPipeDream._UpdateDicts(aDictType: TDLType; aDocID, aSubID: TSubID;
                                 aName: Tl3CustomString): Boolean;
var
 l_S: AnsiString;
begin
 ...
 begin
  Result:= False;
  if aDictType <> dlPrefixes then
  begin
   if aDictType <> dlNone then
  ...
  case aFileType of
   ...
   dd_itKT:
    begin
     ...
     TCustomNSRCReader(f_FileReader).CodePage:= GlobalHtServer.BaseLanguage[CurrentFamily].OEMCodePage;
     TCustomNSRCReader(f_FileReader).AnsiCodePage:= GlobalHtServer.BaseLanguage[CurrentFamily].AnsiCodePage;
    end;
   dd_itEverest,
...
procedure TPipeDream.GenerateDocId(var DocID: TDocID; IsVAS: boolean);
begin
...
class function TPipeDream.GetInputFileType(aFilename: AnsiString; aValidTypes:
    TddPipeInputFileTypeSet): TddPipeInputFileType;
var
 l_Ext: AnsiString;
...
function TPipeDream._MoveDoneFile(const aFileName: AnsiString): Boolean;
begin
 Result:= True;
 // переместить обработанный файл, чтобы избежать повторной заливки
end;

function TBaseImportPipe.Execute: Boolean;
begin
  Result := False;
  if (Reader <> nil) and (Writer <> nil) then
  begin
   if Filter <> nil then
   begin
    Filter.Link(Writer);
    Reader.Generator:= Filter;
   end
   else
    Reader.Generator:= Writer;
   Reader.Execute;
  end; // (Reader <> nil) and (Writer <> nil)
end;

procedure TFileImportPipe.pm_SetFileList(const Value: TStrings);
begin
 if Value <> nil then
 begin
  f_FileList.Assign(Value);
 end;
end;
