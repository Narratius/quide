...
{ $Id: ddImportPipeKernel.pas,v 1.83 2013/04/26 10:21:32 fireton Exp $ }
...
type
 ...
 TddUpdateDictionaryEvent = function (aDictType: TDLType; aDocID, aSubID: TSubID;
  ...
  private
   ...
   function _UpdateDicts(aDictType: TDLType; aSubID: TSubID;
                           aName: Tl3CustomString): Boolean;
  protected
   procedure Cleanup; override;
   procedure CloseStream(NeedUndo : Boolean); override;
  public
   constructor Create(aFamily: Integer; anOwner: Tk2TagGeneratorOwner = nil);
       reintroduce; virtual;
...
Uses
 ...
 k2Base,
 evdBufferedFilter,
 dt_Dict, dt_LinkServ, dt_Serv,
 GarDocBridge, Base_CFG,
 SysUtils
 , k2ForkGenerator,
...
{TddInfoUnitFilter}
function TddInfoUnitFilter.NeedFlushBuffer(const aLeaf: Ik2Tag; aTagId: Integer): Boolean;
var
 lChildID : Integer;
begin
 Result := True;
...
{TddImportPipeFilter}
constructor TddImportPipeFilter.Create(aFamily: Integer; anOwner: Tk2TagGeneratorOwner = nil);
begin
 inherited Create(anOwner);
 f_CustomID := 1;
 { Создание колен }
 f_m3DB := Tm3DB.Make(GlobalHtServer.TextBase[aFamily]);
...
{ TODO 1 -oДудко -cХрень : Разобраться и переделать - это уже давно в настройках указано }
 ...
 if CheckRegion and LinkServer(Family).LinkExists(dlAccGroups, aDocID, l_DictID) then
  Result:= [dcIfNotHasAnno]
 else
  Result:= DeleteConditions;
end;

function TddImportPipeFilter._UpdateDicts(aDictType: TDLType; aSubID: TSubID;
                          aName: Tl3CustomString): Boolean;
begin
 if Assigned(f_OnDictionaryUpdate) then
  Result:= f_OnDictionaryUpdate(aDictType, f_TopicFilter.TopicNo, aSubID, aName)
 else
  Result:= f_UpdateDictionary;
end;

{TddImportFixFilter}
procedure TddImportFixFilter.DoStartChild(TypeID: Long);
var
 l_Type : Tk2Type;
begin
...
begin
 ...
 begin
  if not f_HasActiveInterval and not CurrentType.InheritsFrom(k2_idAnnotopic) then
   AddActiveInterval;
...
begin
 ...
 begin
  ...
  case AtomIndex of
   ...
   k2_tiExternalHandle :
    begin
     if IsEnglish then
      Inc(l_Value.AsInteger, EnglishIncrement);
 ...
 if TT.InheritsFrom(k2_idDictRec) then
 begin
  if TopType[1].InheritsFrom(k2_idPIRec) then
  begin
 ...
 if CurrentType.InheritsFrom(k2_idNumAndDate) and (AtomIndex = k2_tiNumber) then
...
begin
 inherited;
 f_HaveMailed:= DictServer(CurrentFamily).Dict[dlCorSources].FindIDByFullPath(l3PCharLen(PAnsiChar(c_IsMailedStr))) <> cUndefDictID;
end;
