...
{ $Id: dt_Types.pas,v 1.52 2013/04/11 16:46:29 lulin Exp $ }
...
type
 (* Статус действия документа *)
  ...
  PDNType         = ^TDNType;
  TDNType         = (dnDoc,dnPublish,dnMU,dnGD,dnSF,dnChangerDate,dnLawCaseNum);
...
const
 ...
 cctSubDictExpData  = [ctType, ctPrefix, ctClass, {ctKW, Все равно такой команды нет} ctServiceInfo];  //!! см. cSubDictExpData править вместе!!

 cAttrInAnno = [ctAnnoClasses, ctLog_Anno];
 cAttrInDoc  = [Low(TCacheType) .. High(TCacheType)] - cAttrInAnno;
   // принадлежность атрибутов к типам документов (для импорта)
...
type
 TFamTbls     = (ftNone, // фиктивная таблица
                 ftFile,ftHLink,ftSub,ftFree,ftDt1,ftDt2,ftDt3,ftDt5,ftDt5E,
                 ftDt6,ftDt7,ftDT8,ftDt9,ftDtA,ftDtB,ftDTC,ftDtD,ftDtE,ftDt3E,
                 ftDtF,ftDtG,ftDtGE,ftDtI,ftDtJ,
                 ftLnk1,ftLnk2,ftLnk3,ftLnk4,ftLnk5,ftLnk6,
                 ftLnk7,ftLnk8,ftLnk9,ftLnkA,ftLnkB,ftLnkC,ftLnkD,ftLnkE,ftLnkF,
                 ftLnkG,ftLnkH,ftLnkI,ftLnkJ, ftLnkK,
                 ftPriority,ftRenum,ftStage,ftLog,ftActiv,ftAlarm,ftCtrl,
...
Type
  ...
  PDiapType = ^TDiapType;
  TDiapType       = (tdNone,tdSingle,tdGroup,tdAll,tdSearch,tdNumList);
  ...
  TProcessState   = (psStart,psCurrent,psEnd);
...
type
 TepSupportFileType = (outEVD, outEVDtext, outTXT, outNSRC, outRTF, outHTML, outXML);
const
...
type
  TepDivideBy        = (divNone, divTopic, divAccRight, divSize);
...
type
  ...
  TOnLongProcess    = Function (aState : TProcessState) : Boolean of object;
  //TCheckSubInText   = Function (var aSubID : TSubID) : Boolean of object;
