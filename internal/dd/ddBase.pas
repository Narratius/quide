...
// $Id: ddBase.pas,v 1.251 2013/04/11 16:46:27 lulin Exp $
...
type
  ...
  private
    ...
    f_Left: LongInt;
    f_Width: LongInt;
    procedure pm_SetAutoFit(const Value: Boolean);
  protected
    procedure Cleanup; override;
  public
    constructor Create(aOwner: TObject); override;
    procedure Assign(P: TPersistent); override;
    procedure Clear; override;
    property AutoFit: Boolean read f_AutoFit write pm_SetAutoFit;
    property Border: TddBorder read f_Border;
    property Gaph: LongInt read f_Gaph write f_Gaph;
    property Left: LongInt read f_Left write f_Left;
    property Width: LongInt read f_Width write f_Width;
  end;
...
begin
  if Frames[Index].Color > 0 then
    Result:= {Kernel.ColorTable[}Frames[Index].Color//]
  else
    Result:= 0;
end;
...
begin
  ...
  begin
   ...
   try
     for i:= bpTop{Low(TddBorderParts)} to bpRight{High(TddBorderParts)} do
     begin
...
constructor TddRowProperty.Create(aOwner: TObject);
begin
  inherited;
  f_Gaph:= 108;
  f_Left:= -108;
...
procedure TddRowProperty.Assign(P: TPersistent);
var
  C, c1: TddCellProperty;
  i: Integer;
begin
  if (P Is TddRowProperty) then
  begin
    { Здесь нужно присвоить свойства одной таблицы другой }
  //  FBorder.Assign(TddRowProperty(P).Border);
  (* Перенести в Cell
   for i := 0 to TddRowProperty(P).f_CellsProps.Hi do
   begin
    C1 := TddRowProperty(P).Cells[i].Props;
    C := C1.Clone(nil);
    try
     f_CellsProps.Add(C);
    finally
  ...
  *)
   Left:= TddRowProperty(P).Left;
   Gaph:= TddRowProperty(P).Gaph;
   f_Border.Assign(TddRowProperty(P).Border);
  end else
   inherited Assign(P);
end;
...
procedure TddRowProperty.pm_SetAutoFit(const Value: Boolean);
begin
 f_AutoFit := Value;
end;
