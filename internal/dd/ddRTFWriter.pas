...
{ $Id: ddRTFWriter.pas,v 1.152 2013/05/06 12:37:11 dinishev Exp $ }
...
uses
  ...
  k2TagGen,
  Windows, Graphics,
  ddBase, RTFTypes, ddRTFProperties, ddDocument
  , ddCharacterProperty, ddParagraphProperty, ddTextSegment, ddDocumentAtom,
  ddTable, ddTableRow, ddTextParagraph, l3CustomString;
...
begin
  ...
  //       Result:= Result + Format('\brdrcf%d', [f_BorderParts[i].Color]); { цвет }
...
(*
   ...
   begin
    if i > 0 then
     CellsProps[i].Width:= CellsProps[i].Width + CellsProps[Pred(i)].Width
    else
     CellsProps[i].Width:= CellsProps[i].Width + Left;
    Result:= Result+Cell2RTF(CellsProps[i]);
   end; // for i
  end; // with TAP
*)
end;
