Unit ddHTMLAnno;
{ $Id: ddHTMLAnno.pas,v 1.22 2013/04/15 05:59:22 narry Exp $ }
...
Uses
  l3Types, l3Base, l3Filer, l3TempMemoryStream,
  k2Types,
  k2TagGen,
  k2TagTerminator,
  k2DocumentBuffer,
...
type
 ...
 public
  ...
  procedure NewDocDetected(Sender: TObject; TopicNo: Longint; FilerType: Byte; MainGroup: ShortString);
 ...
 protected
   procedure Cleanup; override;
 public
   constructor Create(aOwner: TObject; DocListFileName: AnsiString); reintroduce;
...
type
 ...
 public
   constructor Create(aOwner: TObject; DocListFileName: AnsiString); reintroduce;
   function Execute: Boolean;
   procedure NewDocDetected(Sender: TObject; TopicNo: Longint; FilerType: Byte; MainGroup:
       ShortString);
...
begin
 ...
 if Generator <> nil then
 begin
  if CurrentType.InheritsFrom(k2_idDocument) and (f_DocType = Ord(dtRelText)) then
   WriteBuffer(True);
...
procedure TddHTMLAnnotationFilter.NewDocDetected(Sender: TObject; TopicNo: Longint; FilerType: Byte; MainGroup:
    ShortString);
begin
 case FilerType of
  0: // Document - начинаем накопление. Если есть что-то накопленное, выливаем
   begin
    WriteBuffer(False);
  ...
  1: // Related  - если справка не от накопленного документа, выливаем его
   begin
    CheckBuffer(TopicNo);
  ...
  2: // Annotation - Если есть что-то накопленное, выливаем
   begin
    WriteBuffer(False);
    f_Filtered := False;
    f_IsAnno := True;
...
procedure TddHTMLAnnotationFilter.StartChild(TypeID: Long);
var
 TT, l_Type: Tk2Type;
...
begin
 ...
 begin
  ...
  //f_Filtered:= True;
  f_DocTag := TypeID;
 end; // TT.InheritsFrom(k2_idDocument)
 if f_Filtered then
...
begin
 ...
 begin
  ...
  begin
   ...
   f_Buffer.Root.Write(Generator{, l3_spfAll});
   if not InnerDoc then
    Generator.Finish;
...
constructor TddHTMLAnnotationPipe.Create(aOwner: TObject; DocListFileName: AnsiString);
begin
...
constructor TddHTMLAnnotationPipe1.Create(aOwner: TObject; DocListFileName: AnsiString);
begin
  inherited Create(aOwner);
  FPipe := TSewerPipe.Create(nil);
  FPipe.Convert2OEM := False;
...
procedure TddHTMLAnnotationPipe1.NewDocDetected(Sender: TObject; TopicNo: Longint; FilerType: Byte;
