...
{ $Id: ExportPipe.pas,v 1.201 2013/04/19 13:10:48 lulin Exp $ }
...
type
 TepFilerInfo = record
  Filer   : TevDOSFiler;// Файлер
  FName   : ShortString;// Имя файла
  FileSize: Int64;      // Размер файла
  FirstDoc: Boolean;    // Первая запись в файл
  Pattern : ShortString;// Шаблон имени файла
...
begin
 { TODO : перенести этот кусок непосредственно в арчи }
 (*
...
begin
 ...
 Result:= False;
 ErrorList.Clear;
 if DivideBySize then // корректируем имена файлов
 begin
...
begin
 ...
 Assert(l_Folder <> '', 'Пустое имя папки экспорта');
 case aFiler of
 ...
 if l_NeedNumber then
  ...
  begin
   ...
   //Нельза сравнивать с файлером - в этот момент может быть открыт совсем другой файл!
...
begin
  ...
  *)
  ...
  on E: Exception do
  begin
   l3System.Msg2Log('CRITICAL ERROR: Ошибка (%s) открытия файла %s', [E.Message, l_FileName]);
  end;
 end;
end;
...
var
 ...
 l_NewFile: Boolean;
begin
 // Нужно переключить файлер на новый файл
 ...
 begin // l_Filer <> fileReference
  ...
  if (DivideBy <> divNone) or (f_Filers[l_Filer].Filer.FileName = '') then
  begin
   // Проверить список файлов, чтобы не переоткрывать существуюшие
 ...
 begin
  ...
  try
   ...
   try
    ...
    begin
     if WriteOnePubl(PPublishDataRec(l_PublInDataList.GetItem(I)), l_RangeManager, l_S) then
     begin
...
begin
 ...
 begin
  ...
  if IsTIFF(l_ImageFileName) then // выливаем только TIFF'ы
  begin
